...
describe WhiteKingMovement do
  ...
  describe '#valid?' do
    ...
    context 'when the king is placed in a random position' do
      before do
        board.place_piece(white_king, [3, 4])
      end
  ...
  describe '#checks_own_king?' do
    before do
      board.place_piece(white_king, [1, 0])
      board.place_piece(BlackBishop.new, [1, 1])
    end
    ...
    context 'when it does not put its own king in check' do
      ...
      it 'does not apply the movement' do
        expect(board.piece_at([1, 1])).to be_instance_of(BlackBishop)
      end
    end
  end
