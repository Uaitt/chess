...
describe BlackQueenMovement do
  ...
  describe '::moving?' do
    ...
    context 'when given a BlackQueen' do
      it 'returns true' do
        expect(BlackQueenMovement).to be_moving(BlackQueen.new, end_position)
      end
    end

    context 'when given a WhiteQueen' do
      it 'returns false' do
        expect(BlackQueenMovement).not_to be_moving(WhiteQueen.new, end_position)
      end
    end

    context 'when given a WhitePiece' do
      it 'returns false' do
        expect(BlackQueenMovement).not_to be_moving(WhitePiece.new, end_position)
      end
    end
  end
  ...
  describe '#valid?' do
    context 'when the queen is placed at the top right corner' do
      before do
        board.place_piece(black_queen, [0, 0])
      end
    ...
    context 'when the queen is placed in a random position' do
      before do
        board.place_piece(black_queen, [3, 4])
      end
  ...
  describe '#checks_own_king?' do
    before do
      ...
      board.place_piece(black_queen, [1, 0])
  ...
  describe '#apply' do
    ...
    it 'places the bishop on the right position' do
      expect(board.piece_at([3, 3])).to eq(black_queen)
    end
