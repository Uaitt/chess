...
describe WhiteKnightMovement do
  ...
  describe '::moving?' do
    ...
    context 'when moving a WhiteKnight' do
      it 'returns true' do
        expect(WhiteKnightMovement).to be_moving(WhiteKnight.new, end_position)
      end
    end

    context 'when moving a BlackKnight' do
      it 'returns false' do
        expect(WhiteKnightMovement).not_to be_moving(BlackKnight.new, end_position)
      end
    end

    context 'when moving a BlackPiece' do
      it 'returns false' do
        expect(WhiteKnightMovement).not_to be_moving(BlackPiece.new, end_position)
      end
    end
  end
  ...
  describe '#valid?' do
    context 'when the knight is placed in the top left corner' do
      before do
        board.place_piece(white_knight, [0, 0])
      end
    ...
    context 'when the knight is placed in a random position' do
      before do
        board.place_piece(white_knight, [3, 4])
      end
  ...
  describe '#checks_own_king?' do
    before do
      ...
      board.place_piece(white_knight, [1, 0])
    end
    ...
    context 'when it does not put its own king in check' do
      ...
      it 'does not apply the movement' do
        expect(board.piece_at([1, 0])).to be_instance_of(WhiteKnight)
      end
    end
  end
  ...
  describe '#apply' do
    ...
    it 'places the bishop on the right position' do
      expect(board.piece_at([1, 2])).to eq(white_knight)
    end
