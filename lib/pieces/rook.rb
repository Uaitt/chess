...
class Rook
  ...
  def able_to_reach?(current_position, end_position)
    @possible_moves.each do |move|
      if current_move_matches?(current_position, move, end_position)
        @move = move
        return true
      end
    end
  ...
  def blocked?(board, current_position, end_position)
    blocked_on_its_way_to_final_square?(board, current_position, end_position) ||
  ...
  def blocked_on_its_way_to_final_square?(board, current_position, end_position)
